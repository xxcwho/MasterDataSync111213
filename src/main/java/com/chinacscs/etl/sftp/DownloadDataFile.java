 ...
 * @description 先从本地目录下找到最新下载的.finish文件的时间戳，在和sftp目录中最新导出的.finish文件的时间戳对比，
 * 如果较小，就认为有新的文件，开始执行本下载程序；否则就认为没有新的文件，直接退出本程序
 * 注意：
 * 1.下载是串行下载
...
public class DownloadDataFile {
    private static final Logger logger = LoggerFactory.getLogger(DownloadDataFile.class);
    ChannelSftp sftp;
    Integer subscribeId;
    Integer clientId;
    ...
    int start() {
        ...
        try {
            ...
            //直接从对应的finish文件夹目录下查找最新的finish文件
            logger.debug("远程SFTP目录:{}", remoteSubscribeSftpDir);
            Long maxSftpFinishTimestamp = ((List<String>) (sftp.ls(remoteSubscribeSftpDir + "/finish")
            ...
            //查找所有即将导出的stage表的列表
            String getStageTableInfoListSql = "SELECT STG_TABLE,SUBSCRIBE_LEVEL,SUBSCRIBE_TABLE FROM " + subscribeView + " WHERE ISDEL = 0 AND (CLIENT_ID = ? OR CLIENT_ID IS NULL) AND SUBSCRIBE_ID = ? ";
            ...
            stageTableInfoList.forEach(stageTableInfo -> {
                ...
                String subscribeTable = stageTableInfo.get(2).toString().trim().toLowerCase();
                logger.info("开始下载" + stageTable + "对应的csv文件....");
                //远程分发数据文件目录
                String downloadDir = remoteSubscribeSftpDir + "/" + subscribeTable;
                try {
            ...
            if (errorFlag[0]) {
                ...
                String finishFileDir = remoteSubscribeSftpDir + "/finish";
                ((List<String>) (sftp.ls(finishFileDir + "/*.finish")
                        .stream().map(Object::toString).collect(Collectors.toList())))
                        .stream()
                        .filter(info -> info.endsWith(".finish"))
                        .map(info -> info.split(" +")[8])
                        ...
                        .forEach(fileName -> {
                            try {
                                sftp.get(finishFileDir + "/" + fileName, localDir);
    ...
    public static void main(String[] args) {
        DownloadDataFile downloadFromSftp = new DownloadDataFile();
