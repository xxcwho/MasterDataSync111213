...
public class ExecuteProcedure {
    private static final Logger logger = LoggerFactory.getLogger(ExecuteProcedure.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            logger.info("开始在" + env + "环境执行" + proc);
            if (env.trim().toLowerCase().equals(Constants.ENV_STG)) {
                jdbcManagerStg.executeUpdate(sql, null, true, true);
            } else if (env.trim().toLowerCase().equals(Constants.ENV_TGT) && (!StringUtils.isEmpty(driverTgt))) {
                if (jdbcManagerTgt != null) {
                    jdbcManagerTgt.executeUpdate(sql, null, true, true);
                }
            } else {
                logger.error("错误的执行环境！");
                System.exit(-1);
            }
            logger.info(proc + "执行完毕");
        } catch (ClassNotFoundException e) {
            logger.error("找不到JDBC驱动！");
            LogUtils.logException(logger, e);
            System.exit(2);
        } catch (SQLException e) {
            logger.error(proc + "执行失败！");
            LogUtils.logException(logger, e);
            try {
                String subject = "[S" + subscribeId + "C" + clientId + "]在" + env + "执行" + proc + "失败！";
                String message = "请查看附件中的日志获取详细信息。。";
                alert.sendAlert(subject, message, Arrays.asList(logPath), null);
